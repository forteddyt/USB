...
{
    private JTextField studentFileChosen = new JTextField(),
            studentDir = new JTextField(), mentorFileChosen = new JTextField(),
            mentorDir = new JTextField();
    ...
    private JButton studentOpenButton = new JButton("Choose File"),
            mentorOpenButton = new JButton("Choose File"),
    ...
    public FileChooser()
    {
        try
        {
            ILITEFont = Font.createFont(Font.TRUETYPE_FONT, new File(
                    "src/agency-fb.ttf"));
        } catch (Exception e)
        {
            e.printStackTrace();
    ...
    private class StudentChooserUI extends JPanel
    {
        ...
        public StudentChooserUI()
        {
            ...
            studentFileChosen.setBorder(BorderFactory.createLineBorder(
                    noneGrey, 2));
            FCTemp.add(studentFileChosen, BorderLayout.CENTER);
            FCTemp.add(Box.createRigidArea(new Dimension(30, 10)),
            ...
            JButton mButton = new JButton(new ImageIcon(((new ImageIcon(
                    "src/PurpleFace.png").getImage().getScaledInstance(30, 35,
                    java.awt.Image.SCALE_SMOOTH)))));
            mButtonFormatter.setLayout(new BorderLayout());
            mButtonPanel.setLayout(new BorderLayout());
            mButtonPanel.add(Box.createVerticalStrut(5),
            ...
            mButtonFormatter.add(Box.createHorizontalStrut(5),
                    BorderLayout.LINE_START);
            mButtonPanel.add(mButtonFormatter, BorderLayout.LINE_START);
            mButtonPanel.add(Box.createHorizontalStrut(125 - mButton.getIcon()
                    .getIconWidth() - 5), BorderLayout.LINE_END);

            JPanel qButtonPanel = new JPanel();
            JPanel qButtonFormatter = new JPanel();
            qButtonFormatter.setLayout(new BorderLayout());
            JButton qButton = new JButton(new ImageIcon(((new ImageIcon(
                    "src/QuestionMark.png").getImage().getScaledInstance(30,
                    35, java.awt.Image.SCALE_SMOOTH)))));
            qButtonPanel.setLayout(new BorderLayout());
            qButtonPanel.add(Box.createVerticalStrut(5),
                    BorderLayout.PAGE_START);
            ...
            qButtonFormatter.add(Box.createHorizontalStrut(5),
                    BorderLayout.LINE_END);
            qButtonFormatter.add(qButton, BorderLayout.LINE_START);
            qButtonPanel.add(qButtonFormatter, BorderLayout.LINE_END);
            qButtonPanel.add(Box.createHorizontalStrut(125 - qButton.getIcon()
                    .getIconWidth() - 5), BorderLayout.LINE_START);
            ...
            studentListTitle.setBorder(BorderFactory.createMatteBorder(0, 0, 3,
                    0, ILITEGreen));
            ...
            openButtonFormatter.add(Box.createVerticalStrut(2),
                    BorderLayout.PAGE_START);
            openButtonFormatter.add(Box.createVerticalStrut(2),
                    BorderLayout.PAGE_END);
            openButtonFormatter.add(Box.createHorizontalStrut(2),
                    BorderLayout.LINE_START);
            openButtonFormatter.add(Box.createHorizontalStrut(2),
                    BorderLayout.LINE_END);

            OBTemp.add(openButtonFormatter, BorderLayout.CENTER);
            OBTemp.add(Box.createRigidArea(new Dimension(10, 10)),
                    BorderLayout.LINE_END);
            OBTemp.add(Box.createRigidArea(new Dimension(30, 10)),
                    BorderLayout.LINE_START);
        ...
        public class qAction implements ActionListener
        {
            public void actionPerformed(ActionEvent e)
            {
                if (!iGuide.isVisible())
                {
                    ...
                    iGuide.setLocation(frame.getLocation().x + 15,
                            frame.getLocation().y + 15);
                }
            }
        }
        ...
        public class mAction implements ActionListener
        {
            public void actionPerformed(ActionEvent e)
            {
                if (!mGuide.isVisible())
                {
                    ...
                    mGuide.setLocation(frame.getLocation().x + 15,
                            frame.getLocation().y + 15);
                }
            }
        }
    ...
    private class MentorChooserUI extends JPanel
    {
        public MentorChooserUI()
        {
            ...
            mentorFileChosen.setBorder(BorderFactory.createLineBorder(noneGrey,
                    2));
            ...
            mentorTitle.setBorder(BorderFactory.createMatteBorder(0, 0, 3, 0,
                    ILITEGreen));
            mentorTitle.setHorizontalAlignment(JLabel.CENTER);
            titleFormat.add(Box.createHorizontalStrut(125),
                    BorderLayout.LINE_START);
            titleFormat.add(Box.createHorizontalStrut(125),
                    BorderLayout.LINE_END);
            titleFormat.add(mentorTitle, BorderLayout.CENTER);
            titleFormat.add(Box.createVerticalStrut(15),
                    BorderLayout.PAGE_START);
            ...
            buttonFormatter.add(Box.createHorizontalStrut(2),
                    BorderLayout.LINE_START);
            buttonFormatter.add(Box.createHorizontalStrut(2),
                    BorderLayout.LINE_END);
            buttonFormatter.add(Box.createVerticalStrut(2),
                    BorderLayout.PAGE_START);
            buttonFormatter.add(Box.createVerticalStrut(2),
                    BorderLayout.PAGE_END);
            ...
            buttonField.add(Box.createHorizontalStrut(30),
                    BorderLayout.LINE_START);
            buttonField.add(Box.createHorizontalStrut(10),
                    BorderLayout.LINE_END);
    ...
    private class CodeDataUI extends JPanel
    {
        public CodeDataUI()
        {
            ...
            createCodeFormatter.add(Box.createHorizontalStrut(20),
                    BorderLayout.LINE_START);
            createCodeFormatter.add(Box.createHorizontalStrut(20),
                    BorderLayout.LINE_END);
            createCodeFormatter.add(Box.createVerticalStrut(5),
                    BorderLayout.PAGE_END);
    ...
    private class SaveTo implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent e)
        {
            if (!(studentDir.getText().length() > 0)
                    && !(mentorDir.getText().length() > 0))
            {
                JOptionPane.showMessageDialog(frame,
                        "Select appropriate files.", "Warning",
                        JOptionPane.WARNING_MESSAGE);
            } else if (!(mentorDir.getText().length() > 0))
            {
                JOptionPane.showMessageDialog(frame,
                        "Select a Mentor List file.", "Warning",
                        JOptionPane.WARNING_MESSAGE);
            } else if (!(studentDir.getText().length() > 0))
            {
                JOptionPane.showMessageDialog(frame,
                        "Select a Student List file.", "Warning",
                        JOptionPane.WARNING_MESSAGE);
            } else
            {
                ...
                if (rVal == JFileChooser.APPROVE_OPTION)
                {
                    ...
                    wC = new WebsiteCoder(s, new File(studentDir.getText()),
                            new File(mentorDir.getText()));
                    wC.start();
                }
            }
        }
